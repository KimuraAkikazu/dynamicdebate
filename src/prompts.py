...
最大 {max_turn}ターンの議論で現在{turn}ターン目です。最大ターンまでに必ず結論を導いてください。
...
# 指示
あなたは、議論において次の行動計画を立てる必要があります。
次のターンでのあなたの行動計画をJSON形式で出力してください。
行動計画を立てる際は、このターンでの発言内容をもとに、議論の流れを考慮してください。
...
# あなたの思考プロセス
これまでの議論と、このターンでの発言を受けて、あなたが次に行うべき行動を決定してください。
あなたの決定は、議論に貢献し、あなたの役割を果たすためのものであるべきです。
行動パターンは以下のいずれかです:
- `listen`: 他のエージェントの発言を聞く
- `speak`: 発言する。他のエージェントが発言の途中の場合、割り込んで発言する。
